...
#Concern: implementations in the R language for the Latent Causation algorithm

#Welcome in this instance of code implementing the Latent Causation algorithm, tools for performing simulations and still other features. 
...
#2. Note that some parts of the current implementation are still in a beta version. Parts of the code (as for example the function "causal.interpret()"
...
MCsim=function(model, n_MC_samples, sample_size, obs_var="all", seed=4321, assumed_cause_indicators, assumed_consequence_indicators, indep.measures="all", CI=c(0.025,0.975), boot.pval=TRUE, dcor.R=500, HSIC.B=500, B=1000){
	...
	#n_MC_samples: number of MonteCarlo sample to generate
...
model='
	#No latent confounder and a causal effect from X (uniform) to Y 
...
resu=MCsim(model=model, n_MC_samples=1000, sample_size=400, obs_var=c("x1","x2","x3","y1","y2","y3"), seed=4321, assumed_cause_indicators=c("x1","x2","x3"), assumed_consequence_indicators=c("y1","y2","y3"), indep.measures="all", CI=c(0.025,0.975), boot.pval=TRUE, dcor.R=300, HSIC.B=300, B=1000)
